...
def fetch_all_lines(session_manager: SessionManager) -> List:
    """
...
def create_line_object(line_id: int, session_manager: SessionManager) -> Line:
    ...
    if delta_days < update_frequency_days:
        log("No need to update line {} (ID: {}) as only {} day(s) have passed since last update (expected {})".format(
            line.name,
            line.id,
            delta_days,
